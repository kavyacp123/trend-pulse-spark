          ...
          .map(([theme, prefix]) => {
            return colorConfig
              .map(([key, itemConfig]) => {
                ...
                return color ? `${prefix} [data-chart=${id}] .fill-${key} { fill: ${color}; } ${prefix} [data-chart=${id}] .stroke-${key} { stroke: ${color}; }` : null;
              })
              .join("");
          })
          .join(""),
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || item?.payload?.[key] || item?.color || itemConfig?.color || item?.value;
